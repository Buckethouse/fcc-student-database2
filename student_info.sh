...
echo "$($PSQL "SELECT DISTINCT(course) FROM courses FULL JOIN majors_courses USING(course_id) FULL JOIN majors USING(major_id) FULL JOIN students USING(major_id) WHERE course IS NOT NULL AND (student_id IS NULL OR (first_name = 'Obie' AND last_name = 'Hilpert')) ORDER BY course DESC;")"
